...
class vfsStreamExLockTestCase extends PHPUnit_Framework_TestCase
{
    ...
    {
        @file_put_contents(vfsStream::url('root/testfile'), "some stringn", LOCK_EX);
    ...
    public function flockSouldPass()
    {
        ...
        flock($fp, LOCK_EX);
        fwrite($fp, "another stringn");
        flock($fp, LOCK_UN);
        fclose($fp);
        $this->assertEquals("another stringn", file_get_contents(vfsStream::url('root/testfile')));
    }
}
