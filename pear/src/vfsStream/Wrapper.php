...
class vfsStream_Wrapper
{
     ...
     * vfsStream a vfsStream_Exception will be thrown.
     *
     * @throws  vfsStream_Exception
     */
    public static function register()
    {
        self::$root  = null;
        self::$quota = vfsStream_Quota::unlimited();
        if (true === self::$registered) {
            return;
        }

        if (@stream_wrapper_register(vfsStream::SCHEME, __CLASS__) === false) {
            throw new vfsStream_Exception('A handler has already been registered for the ' . vfsStream::SCHEME . ' protocol.');
        }
     ...
     *
     * @param   vfsStream_Interface_Container  $root
     * @return  vfsStream_Interface_Container
     */
    public static function setRoot(vfsStream_Interface_Container $root)
    {
        self::$root = $root;
        return self::$root;
    }
     ...
     *
     * @param  vfsStream_Quota  $quota
     * @since  1.1.0
     */
    public static function setQuota(vfsStream_Quota $quota)
    {
        self::$quota = $quota;
    }
    ...
    public function stream_open($path, $mode, $options, $opened_path)
    {
        ...
        $this->content = $this->getContentOfType($path, vfsStream_Interface_Content::TYPE_FILE);
    ...
    private function createFile($path, $mode = null, $options = null)
    {
        ...
        $dir = $this->getContentOfType($names['dirname'], vfsStream_Interface_Content::TYPE_DIR);
        if (null === $dir) {
            if (($options & STREAM_REPORT_ERRORS) === STREAM_REPORT_ERRORS) {
    ...
    public function stream_truncate($size)
    {
        ...
        if ($this->content->getType() !== vfsStream_Interface_Content::TYPE_FILE) {
            return false;
        }
     ...
     *
     * @param   string                    $path
     * @param   vfsStream_Abstract_Content  $content
     * @param   Closure                   $change
     * @return  bool
     */
    private function doPermChange($path, vfsStream_Abstract_Content $content, \Closure $change)
    {
    ...
    public function unlink($path)
    {
        ...
        if ($content->getType() !== vfsStream_Interface_Content::TYPE_FILE) {
    ...
    public function rename($path_from, $path_to)
    {
        ...
        if ($dstParentContent->getType() !== vfsStream_Interface_Content::TYPE_DIR) {
    ...
    public function mkdir($path, $mode, $options)
    {
        ...
        if (null === $dir
          || $dir->getType() !== vfsStream_Interface_Content::TYPE_DIR
    ...
    {
        $path  = $this->resolvePath(vfsStream::path($path));
        $child = $this->getContentOfType($path, vfsStream_Interface_Content::TYPE_DIR);
        if (null === $child) {
            return false;
        }
    ...
    {
        $path      = $this->resolvePath(vfsStream::path($path));
        $this->dir = $this->getContentOfType($path, vfsStream_Interface_Content::TYPE_DIR);
