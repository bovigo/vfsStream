...
class vfsStream
{
     ...
     * vfsStream_Wrapper::setRoot(vfsStream::newDirectory('root'));
     * you can simply do
     * vfsStream::setup()
    ...
    {
        vfsStream_Wrapper::register();
        return self::create($structure, vfsStream_Wrapper::setRoot(self::newDirectory($rootDirName, $permissions)));
    }
     ...
     *
     * @param   array               $structure  directory structure to add under root directory
     * @param   vfsStream_Directory  $baseDir    base directory to add structure to
     * @return  vfsStream_Directory
     ...
     * @see     https://github.com/mikey179/vfsStream/issues/20
     */
    public static function create(array $structure, vfsStream_Directory $baseDir = null)
    {
        if (null === $baseDir) {
            $baseDir = vfsStream_Wrapper::getRoot();
        }
     ...
     * @param   vfsStream_Directory  $baseDir    directory to add the structure to
     * @return  vfsStream_Directory
     */
    protected static function addStructure(array $structure, vfsStream_Directory $baseDir)
    {
     ...
     */
    public static function copyFromFileSystem($path, vfsStream_Directory $baseDir = null, $maxFileSize = 1048576)
    {
        if (null === $baseDir) {
            $baseDir = vfsStream_Wrapper::getRoot();
        }
     ...
     * @return  vfsStream_File
     */
    public static function newFile($name, $permissions = null)
    {
        return new vfsStream_File($name, $permissions);
    }
    ...
    public static function newDirectory($name, $permissions = null)
    {
        ...
        if (false === $firstSlash) {
            return new vfsStream_Directory($name, $permissions);
        }

        $ownName   = substr($name, 0, $firstSlash);
        $subDirs   = substr($name, $firstSlash + 1);
        $directory = new vfsStream_Directory($ownName, $permissions);
     ...
     *
     * @param   vfsStream_Interface_Visitor  $visitor  the visitor who inspects
     * @param   vfsStream_Interface_Content  $content  directory structure to inspect
     * @return  vfsStream_Interface_Visitor
     * @throws  \InvalidArgumentException
     * @since   0.10.0
     * @see     https://github.com/mikey179/vfsStream/issues/10
     */
    public static function inspect(vfsStream_Interface_Visitor $visitor, vfsStream_Interface_Content $content = null)
    {
        if (null !== $content) {
            return $visitor->visit($content);
        }

        $root = vfsStream_Wrapper::getRoot();
        if (null === $root) {
    ...
    {
        vfsStream_Wrapper::setQuota(new vfsStream_Quota($bytes));
    }
